...
export default function ShippedOrdersPage() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesCarrier =
      !filters.carrier || order.carrier === filters.carrier;
    ...
    return matchesSearch && matchesDate && matchesCarrier && matchesTotal;
  });
      ...
      <div className="max-w-[1600px] mx-auto p-4 md:p-8 space-y-8">
        ...
        >
          ...
          >
            ...
            >
              Shipped Orders ({filteredOrders.length})
            </motion.h3>
            ...
            <table className="w-full">
              <thead className="bg-(--admin-bg-light) text-(--admin-text-muted) text-[10px] md:text-xs uppercase tracking-wider">
                <tr>
                  ...
                  <th className="p-3 md:p-5 text-left">Shipped Date</th>
                  <th className="p-3 md:p-5 text-left">Total</th>
                  <th className="p-3 md:p-5 text-left hidden md:table-cell">
                    Items
                  </th>
                  <th className="p-3 md:p-5 text-left">Carrier</th>
                  <th className="p-3 md:p-5 text-left">Tracking</th>
                  <th className="p-3 md:p-5 text-left">Est. Delivery</th>
              ...
              <tbody>
                {filteredOrders.map((order) => (
                  ...
                  >
                    ...
                    <td className="p-3 md:p-5">
                      <span className="text-(--admin-gold) font-mono text-xs md:text-sm">
                    ...
                    <td className="p-3 md:p-5">
                      <span
                        className={`px-2 py-1 rounded-full text-[10px] md:text-xs font-bold ${getStatusColor(
                          order.status
                        )}`}
                      >
                        {order.status}
          ...
          <div className="p-4 md:p-6 border-t-(--admin-border) flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-(--admin-text-muted) text-xs md:text-sm">
              Showing {filteredOrders.length} of {orders.length} shipped orders
            </p>
