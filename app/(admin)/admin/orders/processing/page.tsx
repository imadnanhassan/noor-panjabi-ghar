...
export default function ProcessingOrdersPage() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    return matchesSearch && matchesDate && matchesPriority && matchesTotal;
  });
        ...
        >
            ...
            <table className="w-full">
              <thead className="bg-(--admin-bg-light) text-(--admin-text-muted) text-[10px] md:text-xs uppercase tracking-wider">
                <tr>
                  ...
                  <th className="p-3 md:p-5 text-left">Priority</th>
                  <th className="p-3 md:p-5 text-left">Processing Time</th>
              ...
              <tbody>
                {filteredOrders.map((order) => (
                  ...
                  >
                    ...
                    <td className="p-3 md:p-5">
                      <span
                        className={`px-2 py-1 rounded-full text-[10px] md:text-xs font-bold ${getPriorityColor(
                          order.priority
                        )}`}
                      >
                        {order.priority}
          ...
          <div className="p-4 md:p-6 border-t-(--admin-border) flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-(--admin-text-muted) text-xs md:text-sm">
              Showing {filteredOrders.length} of {orders.length} processing
              orders
            </p>
